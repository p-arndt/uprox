<script lang="ts">
	...
	import { formatUsd, formatTokens, formatDateTime, formatDuration } from '$lib/format';
	...
	const rawResponse = $derived(rawResponseBody(selected?.responseBody, selected?.format));
	...
	<div class="space-y-3">
		...
		<div class="flex flex-wrap gap-x-8 gap-y-2 text-sm">
			...
			<div>
				...
				<div class="tabular-nums">{formatDuration(win.end - win.start)}</div>
			</div>
		</div>
	</div>
